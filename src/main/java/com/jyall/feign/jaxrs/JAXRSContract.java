...
//	}
	...
	protected void processAnnotationOnClass(MethodMetadata data, Class<?> clz) {
		...
		if (path != null) {
			...
			if (pathValue.endsWith("/")) {
				// Strip off any trailing slashes, since the template has
				// already had slashes appropriately added
	...
	protected void processAnnotationOnMethod(MethodMetadata data, Annotation methodAnnotation, Method method) {
		...
		if (http != null) {
			...
			// jax-rs allows whitespace around the param name, as well as an
			// optional regex. The contract should
	...
	private void handleConsumesAnnotation(MethodMetadata data, Consumes consumes, String name) {
		...
		data.template().header(CONTENT_TYPE, (String) null); // remove any
																// previous
																// consumes
